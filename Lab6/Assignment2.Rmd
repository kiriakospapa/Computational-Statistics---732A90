...
Likelihood: $$L(f(Y_i,Z_i)) = (\frac{1}{2\lambda^2})^n* \prod_{i = 1}^{n}X_{i}^2*e^{-\frac{\sum_{i = 1}^{n}X_i*Z_i+X_i*2Y_i}{2\lambda}}$$

Loglikelihood: $$Log (L(f(Y_i,Z_i))) = -nlog(2) - 2nlog(\lambda)+\sum_{i = 1}^{n}log(X_i^2)-\frac{\sum_{i = 1}^{n}X_i*Z_i+X_i*2Y_i}{2\lambda}$$

## Step E
To make this easier to work with 

$-\frac{\sum_{i = 1}^{n}X_i*Z_i+X_i*2Y_i}{2\lambda}$
...
EM<-function(x,y,z, kmax){
  ...
  while ((abs(prev_lambda-curr_lambda)>eps) && (k<(kmax+1))){
    ...
    ## E-step ?? Do we even need it?
    #EY= -n*log(2)-2*n*log(prev_lambda)+sum(log(x**2))-(sum(Xobs*Zobs)/2*prev_lambda)-(curr_lambda/prev_lambda)-(sum(x*y)/prev_lambda)
