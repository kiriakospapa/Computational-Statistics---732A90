...
cancel each other and the $\frac{\epsilon}{\epsilon} = 1$. The issue is because of underflow (in both cases). In the first case, R can not represent all the digits. In the second case, in the nominator $100000 + \epsilon$ gives $100000$ as $\epsilon$ is very small and can't be added correctly to 100000.
...
There is a line along the Error = -1, and most of the numbers have a error
between 4 to 0 and -2 to -4. We can therefore conclude that our function
is not accurate.The $\sum_{i = 1}^{n} x^2_i$ is a large value. When we substracting with with $\frac{1}{n}*(\sum_{i = 1}^{n} x_i)^2$ and divide by n we get underflow.
...
prod one in the numerator and one in the denominator.In the first case when k is too big n-k as well, the output will be $\frac{Inf}{Inf}$ as the product of a very big sequence is *Inf* in R. When it happens that in R it returns *NaN* value.

In the second case we have *NaN* when only n-k is a very big number because The output will be $\frac{Inf}{Inf}$. As mentioned above R in that case returns *NaN*


So in the 3rd case it will output *Inf* as both numerator and denominator are inside the same product and it will return Inf for very big numbers and never *NaN*
