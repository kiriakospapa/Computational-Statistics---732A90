...
**A: ** The problem is the missing values at fp(x) from 0 to t_min. In that interval the power law distribution cant generate values(it generates values which are equal to infinitive). Solution for this issue is to find another distribution or line that envelopts the target function.
...
We will implement the above condition in above formula
...
create_plot <- function(i){
  ...
  hist(results[i][results[i] < 200], col="blue", breaks=70, xlab="", ylab="sample density", freq=FALSE, main=title)
}
...
knitr::kable(summarized_results)
```
